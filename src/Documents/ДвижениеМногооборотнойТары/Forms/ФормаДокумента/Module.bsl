...
Функция ПолучитьСписокВидовОпераций()
	...
	ВсеВидыОпераций = Перечисления.Базар_ВидыДвиженийМногооборотнойТары;
	
	Для Каждого ТекПеречисление Из Перечисления.Базар_ВидыДвиженийМногооборотнойТары цикл
...
Процедура ОткрытьДокументВида(ВыбранныйВидОперации)
	...
	ОценкаПроизводительностиКлиент.НачатьЗамерВремени(Истина, "СозданиеФормыДвижениеМногооборотнойТарыОбщая");
	
	ОткрытьФорму("Документ.ДвижениеМногооборотнойТары.Форма.ФормаДокументаОбщая", СтруктураПараметров, ВладелецФормы);
