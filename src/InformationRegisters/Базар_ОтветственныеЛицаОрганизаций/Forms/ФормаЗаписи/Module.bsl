...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если НЕ Запись.ИсходныйКлючЗаписи.Пустой() Тогда
		...
		ОтветственныеЛицаОрганизаций	= РегистрыСведений.Базар_ОтветственныеЛицаОрганизаций.СоздатьМенеджерЗаписи();
		...
		ОтветственныеЛицаОрганизаций.Пользователь		= Запись.ИсходныйКлючЗаписи.Пользователь;
		ОтветственныеЛицаОрганизаций.Прочитать();
		
		СтруктураСтаройЗаписи	= Новый Структура("Период, СтруктурнаяЕдиница, Пользователь, ОтветственноеЛицо, ФизическоеЛицо, Должность, Основание");
	...
	Если ПараметрыЗаписи.Свойство("СтруктураСтаройЗаписи") Тогда
		...
		Если (НЕ ТекущийОбъект.Период = ПараметрыЗаписи.СтруктураСтаройЗаписи.Период
				И (НЕ ТекущийОбъект.ФизическоеЛицо = ПараметрыЗаписи.СтруктураСтаройЗаписи.ФизическоеЛицо
					ИЛИ НЕ ТекущийОбъект.Основание = ПараметрыЗаписи.СтруктураСтаройЗаписи.Основание
...
Процедура ПользовательПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(Запись.Пользователь) тогда
		Запись.ФизическоеЛицо = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.Пользователь,"ФизическоеЛицо");
	КонецЕсли;
...
Процедура ОснованиеПриИзмененииНаСервере()
	...
	Если ЗначениеЗаполнено(Запись.Основание) Тогда
		Запись.Должность = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.Основание, "Должность");
		Запись.Период = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Запись.Основание, "ДатаНачала");
	КонецЕсли;
