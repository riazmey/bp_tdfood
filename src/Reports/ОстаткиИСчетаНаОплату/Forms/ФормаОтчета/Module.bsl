...
Функция ПодготовитьПараметрыОтчета()
	...
	ПараметрыОтчета.Вставить("ОтборГдеЗаказаноЗаполнено"        , Отчет.ОтборГдеЗаказаноЗаполнено);
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = "Остатки и счета на оплату" + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(Отчет.НачалоПериода, Отчет.КонецПериода);
...
Функция ПолучитьПараметрыВыбораЗначенияОтбора() Экспорт
	...
	СписокПараметров.Вставить("Грузополучатель"   , Неопределено);
	СписокПараметров.Вставить("ДоговорКонтрагента", Неопределено);
	СписокПараметров.Вставить("Заказано"          , 0);
	СписокПараметров.Вставить("НачОстаток"        , 0);
	СписокПараметров.Вставить("КонОстаток"        , 0);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбработкаНовостейПереопределяемый.КонтекстныеНовости_ПриСозданииНаСервере(
		ЭтаФорма,
		"БП.Отчет.ОстаткиИСчетаНаОплату",
		"ФормаОтчета",
		НСтр("ru='Новости: Остатки и счета на оплату'"),
...
Процедура ПриОткрытии(Отказ)
	...
	КлючеваяОперация = "ФормированиеОтчетаОстаткиИСчетаНаОплату";
	ВремяНачалаОперации = Неопределено;
