...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.РежимОтчета.СписокВыбора.Очистить();
	Для Каждого Макет из Метаданные.Отчеты.ВедомостьПоПросроченнымЗадолженностям.Макеты цикл
		Элементы.РежимОтчета.СписокВыбора.Добавить(Макет.Имя, Макет.Синоним);
	КонецЦикла;
	...
	Отчет.РежимОтчета = Параметры.РежимОтчета;
	
	Если ПустаяСтрока(Отчет.РежимОтчета) тогда
		Отчет.РежимОтчета = "ДебиторскаяИКредиторскаяЗадолженности";
	КонецЕсли;
		
	Схема = Отчеты.ВедомостьПоПросроченнымЗадолженностям.Создать().ПолучитьМакет(Отчет.РежимОтчета);
...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	ПараметрыОтчета.Вставить("РежимОтчета"                      , Отчет.РежимОтчета);
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	НазваниеСхемы = "";
	Если Отчет.РежимОтчета = "ДебиторскаяИКредиторскаяЗадолженности" тогда
		НазваниеСхемы = " (объединённый)";
	ИначеЕсли Отчет.РежимОтчета = "ДебиторскаяЗадолженность" тогда
		НазваниеСхемы = " (дебиторка)";
	ИначеЕсли Отчет.РежимОтчета = "КредиторскаяЗадолженность" тогда
		НазваниеСхемы = " (кредиторка)";
	КонецЕсли;
	
	Название = "Ведомость по просроченным задолженностям" + НазваниеСхемы;
...
&НаСервере
Процедура РежимОтчетаПриИзмененииНаСервере()
...
Процедура РежимОтчетаПриИзменении(Элемент)
	РежимОтчетаПриИзмененииНаСервере();
КонецПроцедуры
