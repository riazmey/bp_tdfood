...
Функция ПодготовитьПараметрыОтчета()
	...
	Для Каждого СтрокаГруппировки из Отчет.Группировка цикл
		...
		Если НЕ СтрНайти(СтрокаГруппировки.Поле, "ДокументДвижения") = 0 тогда
	...
	Если ЕстьРегистратор тогда
		ПараметрыОтчета.Вставить("СхемаКомпоновкиДанных", Отчеты.ВедомостьПоВзаиморасчетам.ПолучитьМакет("СхемаКомпоновкиДанныхСРегистратором") );
	Иначе
		ПараметрыОтчета.Вставить("СхемаКомпоновкиДанных", Отчеты.ВедомостьПоВзаиморасчетам.ПолучитьМакет("СхемаКомпоновкиДанных") );
	КонецЕсли;
...
Процедура ОбновитьТекстЗаголовка(Форма)
	...
	ЗаголовокОтчета = "Ведомость по взаиморасчетам с контрагентами " + БухгалтерскиеОтчетыКлиентСервер.ПолучитьПредставлениеПериода(Отчет.НачалоПериода, Отчет.КонецПериода);
...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ОбработкаНовостейПереопределяемый.КонтекстныеНовости_ПриСозданииНаСервере(
		ЭтаФорма,
		"БП.Отчет.ВедомостьПоВзаиморасчетам",
		"ФормаОтчета",
		НСтр("ru='Новости: Ведомость по взаиморасчетам с контрагентами'"),
...
Процедура ПриОткрытии(Отказ)
	...
	КлючеваяОперация = "ФормированиеОтчетаВедомостьПоВзаиморасчетамСКонтрагентами";
	ВремяНачалаОперации = Неопределено;
